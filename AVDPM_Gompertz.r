...
data <- data[data$TObs1>0,]
data <- data[data$TObs2>0,]

data <- data[data$ageM+data$trunc>40,]
data <- data[data$ageF+data$trunc>40,]
...
X <- as.data.frame(cbind(data_final$ageM, data_final$ageF))
#X <-  as.data.frame(cbind(data$ageM, data$ageF))
colnames(X) <- c("male", "female")
Z <- cbind(log(as.numeric(data_final$agediff)), data_final$maleelder)
colnames(Z) <- c("agediff", "maleelder")
Z <- as.data.frame(Z)


t1_i <- data_final$TObs1
t2_i <- data_final$TObs2
...
n_iter <- 100000 #number of iterations, set by the researcher
...
## - Covariance gamma prior (parameters of the inverse-Wishart distribution) - lambda3 and coeff subject to sensitivity analysis
lambda3 <- M + 5 # - Keeping the variance at minimum, otherwise just decrease coeff
coeff <- 2
lambda4 <- coeff * diag(1, M) %*% matrix(c(1,0.6,0.6,1), M, M, byrow=TRUE) %*% diag(1, M)

## - prior distribution of m_A (normal distribution and exploit conjugacy)
mean_m_A <- 3
var_m_A <- 0.5

## - prior distribution for s_A (inverse Gamma)
lambda7 <- 3
lambda8 <- 0.5

## - prior distribution for sigma2_A (inverse Gamma)
lambda9 <- 2
lambda10 <- 1

## - prior distribution for zeta_M (parameters of the beta distribution) - subject to sensitivity analysis

mean_beta <- 0.75
sd_beta <- 0.1

lambda11 <- mean_beta * (mean_beta * (1 - mean_beta) / (sd_beta^2) - 1)
lambda12 <- (1 - mean_beta) * (mean_beta * (1 - mean_beta) / (sd_beta^2) - 1)

#### - check
m_beta <- lambda11 / (lambda11 + lambda12)
v_beta <- (lambda11 * lambda12) / (((lambda11 + lambda12)^2) * (lambda11 + lambda12 + 1))

# - phi (reperform lambda5=0.5, lambda6=10)
lambda5 <- 6
lambda6 <- 12

# - Trunc-Norm beta prior
lambda13 <- 0.1
lambda14 <- 0.05
...
psi_sbp[1,K] <- 1
psi_sbp[1,1:(K-1)] <- rbeta(K-1, shape1 = 1, shape2 = phi[1,1], ncp = 0)
...
S_unit_fc <- function(unit){
  ...
  pf_vector <- log(pi_mw[i-1,]) - exp(alpha[i-1,1] + beta[i-1,1] * X$male[unit]   + gamma_s1_lp) * (exp(beta[i-1,1] * (t1_i[unit] + a_i[unit])) - exp(beta[i-1,1] * a_i[unit])) / beta[i-1,1] + d_ci$male[unit]   * gamma_s1_lp - 
                                  exp(alpha[i-1,2] + beta[i-1,2] * X$female[unit] + gamma_s2_lp) * (exp(beta[i-1,2] * (t2_i[unit] + a_i[unit])) - exp(beta[i-1,2] * a_i[unit])) / beta[i-1,2] + d_ci$female[unit] * gamma_s2_lp + 
           dnorm(Z$agediff[unit], mean=gamma_s3_lp, sd=sqrt(z_ad_v[i-1]), log = TRUE) + Z$maleelder[unit] * log(z_star_mo[i-1,]) + (1 - Z$maleelder[unit]) * log(1 - z_star_mo[i-1,])
...
#  }
  ...
  alpha[i,1] <- log(rgamma(1, shape=1 + sum(d_ci$male  ), rate=1 + sum(exp(beta[i-1,1] * X$male   + gamma_star[i-1,(1 + M * (S_unit - 1))]) * (exp((t1_i + a_i) * beta[i-1,1]) - exp(a_i * beta[i-1,1])) / beta[i-1,1])))
  alpha[i,2] <- log(rgamma(1, shape=1 + sum(d_ci$female), rate=1 + sum(exp(beta[i-1,2] * X$female + gamma_star[i-1,(2 + M * (S_unit - 1))]) * (exp((t2_i + a_i) * beta[i-1,2]) - exp(a_i * beta[i-1,2])) / beta[i-1,2])))
  ...
  # Acceptance step
  #start_time <- Sys.time()
  # - Get energy functions + proposals (with truncated normal prior included)
  log_ef <- log(dtruncnorm(beta[i-1,1], mean=beta_star, sd=sigma_beta_pr[1], a=(10^(-5)), b=5)) - log(dtruncnorm(beta_star, mean=beta[i-1,1], sd=sigma_beta_pr[1], a=(10^(-5)), b=5)) + log(dtruncnorm(beta_star, mean=lambda13, sd=lambda14, a=(10^(-5)), b=5)) - log(dtruncnorm(beta[i-1,1], mean=lambda13, sd=lambda14, a=(10^(-5)), b=5))
  
  log_ef <- log_ef + sum( - exp(alpha[i,1] + beta_star   * X$male + gamma_star[i-1,(1 + M * (S_unit - 1))]) * (exp(beta_star   * (t1_i + a_i)) - exp(beta_star   * a_i)) / beta_star   + d_ci$male * (X$male + t1_i + a_i) * (beta_star - beta[i-1,1]) + 
                            exp(alpha[i,1] + beta[i-1,1] * X$male + gamma_star[i-1,(1 + M * (S_unit - 1))]) * (exp(beta[i-1,1] * (t1_i + a_i)) - exp(beta[i-1,1] * a_i)) / beta[i-1,1])
  ...
  log_ef <- log(dtruncnorm(beta[i-1,2], mean=beta_star, sd=sigma_beta_pr[2], a=(10^(-5)), b=5)) - log(dtruncnorm(beta_star, mean=beta[i-1,2], sd=sigma_beta_pr[2], a=(10^(-5)), b=5)) + log(dtruncnorm(beta_star, mean=lambda13, sd=lambda14, a=(10^(-5)), b=5)) - log(dtruncnorm(beta[i-1,2], mean=lambda13, sd=lambda14, a=(10^(-5)), b=5))
  
  log_ef <- log_ef + sum( - exp(alpha[i,2] + beta_star   * X$female + gamma_star[i-1,(2 + M * (S_unit - 1))]) * (exp(beta_star   * (t2_i + a_i)) - exp(beta_star   * a_i)) / beta_star   + d_ci$female * (X$female + t2_i + a_i) * (beta_star - beta[i-1,2]) + 
                            exp(alpha[i,2] + beta[i-1,2] * X$female + gamma_star[i-1,(2 + M * (S_unit - 1))]) * (exp(beta[i-1,2] * (t2_i + a_i)) - exp(beta[i-1,2] * a_i)) / beta[i-1,2])
  ...
  for(k in 1:K){
    ...
    log_sum_Si[k] <- log_sum_Si[k] + sum(ifelse(S_unit==k, (- exp(alpha[i,1] + beta[i,1] * X$male  ) * (exp(beta[i,1] * (t1_i + a_i)) - exp(beta[i,1] * a_i)) / beta[i,1]) * (exp(gamma_proposed[1]) - exp(gamma_star[i-1,ga_k_pos[1]])) + 
                                                           (- exp(alpha[i,2] + beta[i,2] * X$female) * (exp(beta[i,2] * (t2_i + a_i)) - exp(beta[i,2] * a_i)) / beta[i,2]) * (exp(gamma_proposed[2]) - exp(gamma_star[i-1,ga_k_pos[2]])) + d_ci$male * (gamma_proposed[1] - gamma_star[i-1,ga_k_pos[1]]) + d_ci$female * (gamma_proposed[2] - gamma_star[i-1,ga_k_pos[2]]), 0))
    ...
    mean_zeta_ad_p <- (sum(ifelse(S_unit==k,1,0) * Z$agediff) / z_ad_v[i-1] + mu_zeta[i-1] / sigma2_zeta[i-1]) * v_zeta_ad_p  #  - beta1[i,1] * X1_cov[,1] - beta1[i,2] * X1_cov[,2] - beta1[i,3] * X1_cov[,3] - beta1[i,4] * X1_cov[,4] - beta1[i,5] * X1_cov[,5] - beta1[i,6] * X1_cov[,6] - beta1[i,7] * X1_cov[,7])
    
    z_star_ad[i,k] <- rnorm(1, mean=mean_zeta_ad_p, sd=sqrt(v_zeta_ad_p))
    
    # - Update zeta_star_mo
    z_star_mo[i,k] <- rbeta(1, shape1 = (lambda11 + sum(Z$maleelder * ifelse(S_unit==k,1,0))), shape2 = (lambda12 + sum((1 - Z$maleelder) * ifelse(S_unit==k,1,0))))
  ...
  Lambda8 <- 1/(1/lambda8 + N_c / sigma2_zeta[i-1])
  ...
  Lambda7 <- Lambda8 * (mu_zeta_sum_over_k / sigma2_zeta[i-1] + mean_m_A / var_m_A)
  ...
  for(k in 1:K){
      means_cross_prod <- means_cross_prod + ((z_star_ad[i,k] - mu_zeta[i])^2) * ifelse(N_k[k]>0,1,0) 
  }
  
  Lambda9 <- N_c/2 + lambda7
  Lambda10 <- lambda8 + 0.5 * means_cross_prod
  ...
  suff_stat <- 0.5 * sum((Z$agediff - z_star_ad[i,S_unit])^2)
  z_ad_v[i] <- 1 / rgamma(1, shape = (lambda9 + 0.5 * sample_size), rate = (lambda10 + suff_stat) )
  ...
  if(i %in% seq(5000, n_iter, 5000)){
    save.image("/Users/francesco/Dropbox/AVDPM/20240503/AVDPM_r1_ABC.RData")
  }
  
}

plot(c(1:i), alpha[1:i,1], type='l', ylim=c(-10,0))
plot(c(1:i), alpha[1:i,2], type='l', ylim=c(-10,0))

plot(c(1:i), beta[1:i,1], type='l',ylim=c(0,0.2))
plot(c(1:i), beta[1:i,2], type='l',ylim=c(0,0.2))


plot(c(1:n_iter), rowSums(N_k_tb * log(ifelse(N_k_tb>0,N_k_tb,1))), type='l')
